...
export default {
  ...
  getters: {
    openInspections: (state) =>
      state.inspections.filter((i) => i.status === 'OPEN'),
    reviewInspections: (state) =>
      state.inspections.filter((i) => i.status === 'FOR_REVIEW'),
    completedInspections: (state) =>
      state.inspections.filter((i) => i.status === 'COMPLETED'),
  },
  mutations: {
